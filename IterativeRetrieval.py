...
def Generate_MERRA2(year='2016',ARMStation='McMurdo',LatRange=[-90,-60],LonRange=[-180,180]):
    ...
    return        
# =================================================================================================================
def forwardRT( X, input_CLOUD=None, input_QI=None, input_QL=None, pressure=None, zenithAngleATMS=0, latitude=0, longitude=0, date=np.array([2015, 8, 1, 0, 0, 0]), surfacepressure=1013., surfacetemperature=0.263178E+03, snow_frac=0.0, surfaceType=np.array([0, 1]), FASTEM='New ice (snow)', temperature2M=None, humidity2M=None, windu2M=None, windv2M=None, skintemperature=None, emiss=None  ):
    global ErrorRTTOV
    ...
    nprofiles = 1
    myProfilesATMS  = pyrttov.ProfilesScatt(nprofiles, nlevels)
    ...
    if True:
        ...
        #----------------------
        myProfilesATMS.Ph = np.empty((1,nlevels+1))
        
        myProfilesATMS.Ph[0,nlevels]    = surfacepressure/1e2
        for i in np.arange(1,nlevels)[::-1]:
            myProfilesATMS.Ph[0,i]      = (myProfilesATMS.P[0,i] + myProfilesATMS.P[0,i-1])/2.0
        myProfilesATMS.Ph[0,0]  = 0.0
        
        myProfilesATMS.HydroFrac = np.empty((1,nlevels))
        myProfilesATMS.Clw = np.empty((1,nlevels))
        myProfilesATMS.Ciw = np.empty((1,nlevels))
        myProfilesATMS.Snow = np.empty((1,nlevels))
        myProfilesATMS.Rain = np.empty((1,nlevels))
                
        CLOUD   = list(input_CLOUD) + [input_CLOUD[71]]
        QI      = list(input_QI) + [input_QI[71]]
        QL      = list(input_QL) + [input_QL[71]]
        
        CLOUD   = np.array(CLOUD)
        QI      = np.array(QI)
        QL      = np.array(QL)        
        
        myProfilesATMS.HydroFrac[0,:]   = np.copy(CLOUD)
        myProfilesATMS.Clw[0,:]         = np.copy(QL)
        myProfilesATMS.Ciw[0,:]         = np.copy(QI)
        myProfilesATMS.Snow[0,:]        = 0
        myProfilesATMS.Rain[0,:]        = 0
        
        #----------------------
        myProfilesATMS.Angles   = np.empty((1,2))
        myProfilesATMS.S2m      = np.empty((1,5))
        myProfilesATMS.Skin     = np.empty((1,8))
        myProfilesATMS.SurfType = np.empty((1,))
        myProfilesATMS.SurfGeom = np.empty((1,3))
        myProfilesATMS.DateTimes= np.empty((1,6))
        
        myProfilesATMS.Angles[0,:]      = np.array([zenithAngleATMS, 0.0])
        myProfilesATMS.S2m[0,:]         = np.array([surfacepressure/1e2, temperature2M, humidity2M, windu2M, windv2M])
        myProfilesATMS.SurfType[0]      = 1
    ...
    ATMSRttov.FileHydrotable = 'rtcoef_rttov13/hydrotable/hydrotable_jpss_atms.dat'
    
    ATMSRttov.Options.AddInterp = True
    ATMSRttov.Options.StoreTrans = True
    ATMSRttov.Options.VerboseWrapper = True
    ATMSRttov.Options.LuserCfrac = False
    ...
    if (ErrorRTTOV==False):    
        ...
        surfemisrefl_ATMS = np.zeros((2,nprofiles,nchan_ATMS), dtype=np.float64)
        surfemisrefl_ATMS[:,:,:] = -1.0     
...
def runRetrieval(year='2016',month='06',ARMStation='McMurdo',appendix=1,E0=0.8):
    ...
    if True:
        ...
        MERRA2['3D_CLOUD']     = np.copy(hf[ARMStation+'/'+month+'/MERRA2_3D_CLOUD'])
        MERRA2['3D_QI']     = np.copy(hf[ARMStation+'/'+month+'/MERRA2_3D_QI'])
        MERRA2['3D_QL']     = np.copy(hf[ARMStation+'/'+month+'/MERRA2_3D_QL'])
        ...
        MERRA2['3D_QI']   = MERRA2['3D_QI'][sorta,:]
        MERRA2['3D_QL']   = MERRA2['3D_QL'][sorta,:]
        MERRA2['3D_CLOUD']   = MERRA2['3D_CLOUD'][sorta,:]        
            ...
            for itime in range(np.size(ATMS_Input_Time)):
                ...
                while ( (iter<=10) & (keep_iter==True) ):
                    ...
                    if True:
                        ...
                        forwardKwArgs = dict(
                            input_CLOUD=CLOUD_3D[itime,:],
                            input_QI=QI_3D[itime,:], 
                            input_QL=QL_3D[itime,:], 
                            pressure=inputP,
